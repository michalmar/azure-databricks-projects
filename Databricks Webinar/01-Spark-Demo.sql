...
-- MAGIC %python
-- MAGIC 
-- MAGIC from pyspark.sql.functions import col, floor, translate, round
-- MAGIC from pyspark.ml import Pipeline
-- MAGIC from pyspark.ml.feature import VectorAssembler, OneHotEncoder
-- MAGIC from pyspark.ml.regression import LinearRegression
-- MAGIC 
-- MAGIC inputDF = (spark.read.table("AirlineFlight")
-- MAGIC   .withColumn("HourOfDay", floor(col("CRSDepTime")/100))
-- MAGIC   .withColumn("DepDelay", translate(col("DepDelay"), "NA", "0").cast("integer")))
-- MAGIC 
-- MAGIC (trainingDF, testDF) = inputDF.randomSplit([0.80, 0.20], seed=999)
-- MAGIC 
-- MAGIC pipeline = Pipeline(stages=[
-- MAGIC     OneHotEncoder(inputCol="HourOfDay", outputCol="HourVector"),
-- MAGIC     VectorAssembler(inputCols=["HourVector"], outputCol="Features"),
-- MAGIC     LinearRegression(featuresCol="Features", labelCol="DepDelay", predictionCol="DepDelayPredicted", regParam=0.0)
-- MAGIC   ])
-- MAGIC 
-- MAGIC model = pipeline.fit(trainingDF)
-- MAGIC resultDF = model.transform(testDF)
-- MAGIC resultDF.createOrReplaceTempView("Predictions")
...
select Year,
       ...
       FlightNum,
       DepDelay,
       round(DepDelayPredicted, 2) as DepDelayPredicted
from Predictions

-- COMMAND ----------

select HourOfDay,
       round(avg(DepDelay), 3) as Actual,
       round(avg(DepDelayPredicted), 3) as Predicted
from Predictions
group by HourOfDay
order by HourOfDay
...
-- MAGIC %python
-- MAGIC 
-- MAGIC from pyspark.sql.functions import col, date_format, unix_timestamp, window
-- MAGIC from pyspark.sql.types import StructType
-- MAGIC 
-- MAGIC spark.conf.set("spark.sql.shuffle.partitions", "8")
-- MAGIC 
-- MAGIC flightSchema = (StructType()
-- MAGIC   .add("FlightNumber", "integer")
-- MAGIC   .add("DepartureTime", "string")
-- MAGIC   .add("Delay", "double")
-- MAGIC   .add("Airline", "string")
-- MAGIC )
-- MAGIC streamingDF = (spark.readStream
-- MAGIC   .schema(flightSchema)
-- MAGIC   .csv(DummyDataGenerator.streamDirectory)
-- MAGIC   .withColumn("DepartureTime", unix_timestamp("DepartureTime", "yyyy-MM-dd'T'HH:mm:ss").cast("timestamp"))
-- MAGIC   .withWatermark("DepartureTime", "5 minute")
-- MAGIC )
-- MAGIC streamingDF.createOrReplaceTempView("FlightStream")

-- COMMAND ----------

select window.start as Time,
       Airline,
       avg(delay) as `Average Delay`
from FlightStream
group by window(DepartureTime, "15 seconds"), Airline
order by Time, Airline
...
-- MAGIC To visualize the results results above, click on the plot button directly below the results above, then click the Plot Options button set:
-- MAGIC * Display Type: Line Chart
-- MAGIC * Values: Average Delay
-- MAGIC * Series Groupings: Airline
-- MAGIC * Keys: Time
-- MAGIC 
-- MAGIC And resize the plot to be taller and wider by dragging the triangle in the bottom-right of the plot.
...
-- MAGIC 
-- MAGIC try:
-- MAGIC   from graphframes import GraphFrame
-- MAGIC except ModuleNotFoundError:
-- MAGIC   try:
-- MAGIC     dbutils.library.installPyPI("graphframes")
-- MAGIC   except:
-- MAGIC     raise Exception("The GraphFrames library must be installed for this to run.  Please read the example below rather than run it.")
-- MAGIC 
-- MAGIC from pyspark.sql.functions import col, concat_ws, round
-- MAGIC 
-- MAGIC flightVerticesDF = (spark.read
-- MAGIC   .option("header", True)
-- MAGIC   .option("delimiter", "\t")
-- MAGIC   .csv("dbfs:/mnt/training/asa/airport-codes/airport-codes.txt")
-- MAGIC   .withColumnRenamed("IATA", "id"))
-- MAGIC 
-- MAGIC flightEdgesDF = (spark.table("Databricks.AirlineFlight")
-- MAGIC   .withColumnRenamed("Origin", "src")
-- MAGIC   .withColumnRenamed("Dest", "dst"))
-- MAGIC 
-- MAGIC flightGF = GraphFrame(flightVerticesDF, flightEdgesDF)
-- MAGIC pageRankDF = flightGF.pageRank(tol=0.05)
-- MAGIC 
-- MAGIC resultsDF = (pageRankDF.vertices
-- MAGIC   .select(concat_ws(", ", col("city"), col("state")).alias("Location"),
-- MAGIC           round(col("pagerank"), 1).alias("Rank"))
-- MAGIC   .orderBy(col("pagerank").desc()))
-- MAGIC 
-- MAGIC display(resultsDF)
